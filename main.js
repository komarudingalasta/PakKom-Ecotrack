...
async function provisionStudentAccounts(){
  if(state.profile.role!=='admin')return; const candidates=state.students.filter(s=>s.active!==false&&!s.authUid); if(!candidates.length)return toast('Semua siswa aktif sudah memiliki akun.'); if(!confirm(`Buat akun login untuk ${candidates.length} siswa? Username dan password menggunakan NIS. Proses dapat memerlukan waktu.`))return;
  ...
  for(const st of candidates){const nis=String(st.nis||st.id||'').trim();if(nis.length<6){fail++;continue}try{const cred=await createUserWithEmailAndPassword(a,nis.toLowerCase().replace(/[^a-z0-9._-]/g,'')+'@pakkom-ecotrack.app',nis);await setDoc(doc(db,'users',cred.user.uid),{role:'siswa',active:true,approved:true,name:st.name||'',loginId:nis,nis,studentId:st.id,classId:st.classId||'',createdAt:new Date().toISOString()});await setDoc(doc(db,'students',st.id),{authUid:cred.user.uid},{merge:true});await a.signOut();ok++}catch(e){console.warn('student provision',nis,e);fail++}}
  await deleteApp(secondary).catch(()=>{});toast(`Akun siswa: ${ok} berhasil, ${fail} gagal. NIS kurang dari 6 digit tidak dapat menjadi password Firebase.`);master();
}
