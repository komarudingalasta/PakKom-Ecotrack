...
let user=null,profile=null,tasks=[],programs=[],assessmentCuts=[...DEFAULT_CUTS],groupMaxMembers=6;
...
async function load(){await Promise.all([loadAssessment(),loadGroupSettings()]);const [snap,ps]=await Promise.all([db.collection('assignments').get(),db.collection('cocurricularPrograms').get()]);programs=ps.docs.map(d=>({id:d.id,...d.data()})).filter(x=>x.archived!==true);tasks=snap.docs.map(d=>({id:d.id,...d.data()})).filter(t=>t.archived!==true).sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||'')));draw()}
...
function draw(){
  ...
  $('#view').innerHTML=`${admin?`<div class="toolbar"><button id="newProgram" class="secondary">+ Program Kokurikuler</button><button id="assessmentSettings" class="secondary">Atur Rentang Nilai</button><button id="groupSettings" class="secondary">Atur Maks. Anggota</button><button id="activityLog" class="secondary">Riwayat Aktivitas</button></div>`:''}${sections||'<div class="card empty">Belum ada tugas.</div>'}`;
  if(admin){$('#assessmentSettings').onclick=openAssessmentSettings;$('#groupSettings').onclick=openGroupSettings;$('#newProgram').onclick=openProgramBuilder;$('#activityLog').onclick=openActivityLog}
...
function openGroupSettings(){
  modal(`<h2>Atur Maksimal Anggota Kelompok</h2><p class="meta">Batas ini berlaku untuk pembuatan kelompok baru dan saat Admin mengubah anggota. Kelompok lama yang jumlah anggotanya melebihi batas baru tidak diubah otomatis.</p><div class="field"><label>Maksimal anggota per kelompok</label><input id="groupMaxMembers" type="number" min="2" max="36" value="${groupMaxMembers}"><small>Minimal 2, maksimal 36 siswa.</small></div><div class="notice">Saat ini: maksimal <b>${groupMaxMembers} siswa</b> per kelompok.</div><div class="actions"><button id="saveGroupSettings" class="primary">Simpan</button></div>`);
  $('#saveGroupSettings').onclick=async()=>{const n=Number($('#groupMaxMembers').value);if(!Number.isInteger(n)||n<2||n>36)return alert('Maksimal anggota harus bilangan 2 sampai 36.');try{await db.collection('settings').doc('taskGroup').set({maxMembers:n,updatedAt:new Date().toISOString(),updatedByUid:user.uid,updatedByName:profile.name||''},{merge:true});groupMaxMembers=n;close();draw();alert(`Maksimal anggota kelompok diubah menjadi ${n} siswa.`)}catch(e){alert('Gagal menyimpan pengaturan: '+e.message)}}
}
