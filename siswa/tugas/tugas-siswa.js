...
function draw(){
  ...
  $('#view').innerHTML=list.length?`<div class="grid">${list.map(t=>{const s=sm.get(t.id),st=statusOf(t,s);return `<article class="card task"><div class="tags"><span class="tag">${t.taskType==='group'?'Kelompok':'Individu'}</span><span class="tag status-${st}">${statusLabel(st)}</span></div><h3>${esc(t.title)}</h3><p>${esc(t.description||'')}</p><div class="meta">Batas: ${esc(dueText(t))}</div>${s?.feedback?`<div class="notice"><b>Catatan guru:</b> ${esc(s.feedback)}</div>`:''}${st==='graded'?`<div class="meta"><b>Nilai ${esc(s.score??'-')} • ${esc(scoreCategory(s.score)||'-')}</b></div>`:''}<button class="primary" data-open="${t.id}">${st==='graded'?'Lihat Nilai':st==='submitted'?'Lihat Pengumpulan':st==='revision'?'Perbaiki Tugas':'Kerjakan'}</button></article>`}).join('')}</div>`:'<div class="card empty">Tidak ada tugas pada kategori ini.</div>';
...
function openTask(id){
  ...
  modal(`<h2>${esc(t.title)}</h2><p>${esc(t.description||'')}</p><div class="meta">Batas: ${esc(dueText(t))}</div>${fields}<div class="notice"><small>Foto, video, dan dokumen dikumpulkan melalui link. Jika menggunakan Google Drive, pastikan guru dapat membuka file tanpa meminta izin akses.</small></div>${s?.feedback?`<div class="notice"><b>Catatan guru:</b> ${esc(s.feedback)}</div>`:''}${st==='graded'?`<div class="card"><b>Nilai: ${esc(s.score??'-')}</b><div class="meta">Kategori: <b>${esc(scoreCategory(s.score)||'-')}</b></div></div>`:st==='submitted'?`<div class="notice"><b>Sudah dikumpulkan.</b><br><small>${esc(s.submittedAt||'')}</small></div>`:`<div class="actions"><button id="draft" class="secondary">Simpan Draf</button><button id="submit" class="primary">${st==='revision'?'Kumpulkan Ulang':'Kumpulkan'}</button></div>`}`);
