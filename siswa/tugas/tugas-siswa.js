...
function openTask(id){
  ...
  const st=statusOf(t,s),locked=st==='graded'||st==='submitted',old=s?.answers?.[0]||{},type=t.answerType||'link';
  modal(`<h2>${esc(t.title)}</h2><p>${esc(t.description||'')}</p><div class="meta">Batas: ${esc(dueText(t))}</div>${type==='link'?`<div class="field"><label>Link jawaban / bukti</label><input id="answer" type="url" placeholder="https://..." value="${esc(old.url||'')}" ${locked?'disabled':''}></div><div class="field"><label>Keterangan</label><textarea id="desc" ${locked?'disabled':''}>${esc(old.description||'')}</textarea></div><div class="notice"><small>Jika menggunakan Google Drive, pastikan guru dapat membuka link tanpa meminta izin akses.</small></div>`:`<div class="field"><label>Jawaban</label><textarea id="answer" ${locked?'disabled':''}>${esc(old.text||'')}</textarea></div>`}${s?.feedback?`<div class="notice"><b>Catatan guru:</b> ${esc(s.feedback)}</div>`:''}${st==='graded'?`<div class="card"><b>Nilai: ${esc(s.score??'-')}</b></div>`:st==='submitted'?`<div class="notice"><b>Sudah dikumpulkan.</b><br><small>${esc(s.submittedAt||'')}</small></div>`:`<div class="actions"><button id="draft" class="secondary">Simpan Draf</button><button id="submit" class="primary">${st==='revision'?'Kumpulkan Ulang':'Kumpulkan'}</button></div>`}`);
  if(!locked){$('#draft').onclick=()=>save(t,s,'draft');$('#submit').onclick=()=>save(t,s,'submitted')}
}
async function save(t,s,status){
  const type=t.answerType||'link',v=$('#answer').value.trim(); if(status==='submitted'&&!v)return alert('Jawaban wajib diisi.');
  if(type==='link'&&v){try{new URL(v)}catch(_){return alert('Link belum valid. Awali dengan https://')}}
  const ans=type==='link'?{url:v,description:$('#desc').value.trim()}:{text:v},now=new Date().toISOString();
  const ref=s?db.collection('taskSubmissions').doc(s.id):db.collection('taskSubmissions').doc(`${t.id}_${profile.loginId}`);
  const data={taskId:t.id,taskType:'individual',studentUid:user.uid,studentId:profile.studentId||'',nis:String(profile.loginId),studentName:profile.name||'',classId:norm(profile.classId),groupNo:null,groupKey:'',memberNis:[],memberNames:[],answers:[ans],reflection:'',status,submittedAt:status==='submitted'?now:(s?.submittedAt||null),submittedByNis:status==='submitted'?String(profile.loginId):(s?.submittedByNis||''),submittedByName:status==='submitted'?(profile.name||''):(s?.submittedByName||''),updatedAt:now,updatedByNis:String(profile.loginId)};
