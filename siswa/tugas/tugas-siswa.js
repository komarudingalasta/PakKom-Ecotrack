...
function subForTask(t){if(t.taskType==='group'){const g=groupForTask(t.id);return g?subs.find(s=>s.taskId===t.id&&s.groupKey===g.groupKey):null}return subs.find(s=>s.taskId===t.id&&String(s.nis)===String(profile.loginId))}
...
async function load(){
  ...
  try{const member=await db.collection('taskSubmissions').where('memberNis','array-contains',nis).get();member.docs.forEach(d=>map.set(d.id,{id:d.id,...d.data()}))}catch(e){errors.push('pengumpulan kelompok: '+(e.code||e.message))}
  subs=[...map.values()];
  try{const gs=await db.collection('taskGroups').where('classId','==',classId).get();groups=gs.docs.map(d=>({id:d.id,...d.data()})).filter(g=>Array.isArray(g.memberNis)&&g.memberNis.map(String).includes(nis))}catch(e){groups=[];errors.push('kelompok: '+(e.code||e.message))}
